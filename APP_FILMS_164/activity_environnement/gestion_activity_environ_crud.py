...
@app.route("/activity_environ_afficher/<int:id_film_sel>", methods=['GET', 'POST'])
def activity_environ_afficher(id_film_sel):
    print(" activity_environ_afficher id_film_sel ", id_film_sel)
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT environ_name,name_activity,
            GROUP_CONCAT(DISTINCT activity_environ_id ORDER BY activity_environ_id) as ActivityEnvironIDs,
            GROUP_CONCAT(DISTINCT name_activity ORDER BY name_activity) as ActivityEnviron
            ...
            raise ExceptionFilmsGenresAfficher(f"fichier : {Path(__file__).name}  ;  {activity_environ_afficher.__name__} ;"
                                               f"{Exception_films_genres_afficher}")

    print("films_genres_afficher  ", data_genres_films_afficher)
    # Envoie la page "HTML" au serveur.
    return render_template("activity_environnement/activity_environ_afficher.html", data=data_genres_films_afficher)
...
def edit_activity_environ_selected():
    if request.method == "GET":
        try:
            ...
            data_genre_film_selected, data_genres_films_non_attribues, data_genres_films_attribues = \
                activity_environ_afficher_data(valeur_id_film_selected_dictionnaire)
    ...
    return render_template("activity_environnement/activity_environ_modifier_tags_dropbox.html",
...
def update_activity_environ_selected():
    if request.method == "POST":
        try:
            ...
            # "fk_environ"/"environ_id" et "fk_activity"/"activity_id" dans la "t_activity_environ"
    ...
    return redirect(url_for('activity_environ_afficher', id_film_sel=id_film_selected))
...
def activity_environ_afficher_data(valeur_id_film_selected_dict):
    print("valeur_id_film_selected_dict...", valeur_id_film_selected_dict)
    try:

        strsql_film_selected = """SELECT environ_id, environ_name ,idActivityEnviron, GROUP_CONCAT(activity_id) as ActivityEnviron FROM t_activity_environ
                                        ...
                                        WHERE environ_id = %(value_id_film_selected)s GROUP BY environ_id, environ_name
"""
        ...
        strsql_genres_films_attribues = """SELECT environ_id, activity_id, name_activity ,idActivityEnviron FROM t_activity_environ
                                            INNER JOIN t_environ ON t_environ.environ_id = t_activity_environ.fk_environ
                                            INNER JOIN t_activity ON t_activity.activity_id = t_activity_environ.fk_activity
                                            WHERE environ_id = %(value_id_film_selected)s"""
        ...
        raise ExceptionGenresFilmsAfficherData(f"fichier : {Path(__file__).name}  ;  "
                                               f"{activity_environ_afficher_data.__name__} ; "
