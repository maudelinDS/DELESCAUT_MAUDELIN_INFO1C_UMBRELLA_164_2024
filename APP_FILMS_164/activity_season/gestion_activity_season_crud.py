...
@app.route("/activity_season_afficher/<int:id_film_sel>", methods=['GET', 'POST'])
def activity_season_afficher(id_film_sel):
    print(" activity_season_afficher id_film_sel ", id_film_sel)
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT season_name,name_activity,
            GROUP_CONCAT(DISTINCT activity_season_id ORDER BY activity_season_id) as ActivitySeasonIDs,
            GROUP_CONCAT(DISTINCT name_activity ORDER BY name_activity) as ActivitySeason
            ...
            raise ExceptionFilmsGenresAfficher(f"fichier : {Path(__file__).name}  ;  {activity_season_afficher.__name__} ;"
                                               f"{Exception_films_genres_afficher}")

    print("films_genres_afficher  ", data_genres_films_afficher)
    # Envoie la page "HTML" au serveur.
    return render_template("activity_season/activity_season_afficher.html", data=data_genres_films_afficher)
...
def edit_activity_season_selected():
    if request.method == "GET":
        try:
            ...
            data_genre_film_selected, data_genres_films_non_attribues, data_genres_films_attribues = \
                activity_season_afficher_data(valeur_id_film_selected_dictionnaire)
    ...
    return render_template("activity_season/activity_season_modifier_tags_dropbox.html",
...
def update_activity_season_selected():
    if request.method == "POST":
        try:
            ...
            # "fk_season"/"season_id" et "fk_activity"/"activity_id" dans la "t_activity_season"
            strsql_insert_genre_film = """INSERT INTO t_activity_season (activity_season_id,fk_activity, fk_season)
    ...
    return redirect(url_for('activity_season_afficher', id_film_sel=id_film_selected))
...
def activity_season_afficher_data(valeur_id_film_selected_dict):
    print("valeur_id_film_selected_dict...", valeur_id_film_selected_dict)
    try:

        strsql_film_selected = """SELECT season_id, season_name ,idActivitySeason, GROUP_CONCAT(activity_id) as ActivitySeason FROM t_activity_season
                                        ...
                                        WHERE season_id = %(value_id_film_selected)s GROUP BY season_id, season_name, score
"""
        ...
        strsql_genres_films_attribues = """SELECT season_id, activity_id, name_activity ,idActivitySeason FROM t_activity_season
                                            INNER JOIN t_season ON t_season.season_id = t_activity_season.fk_season
                                            INNER JOIN t_activity ON t_activity.activity_id = t_activity_season.fk_activity
                                            WHERE season_id = %(value_id_film_selected)s"""
        ...
        raise ExceptionGenresFilmsAfficherData(f"fichier : {Path(__file__).name}  ;  "
                                               f"{activity_season_afficher_data.__name__} ; "
