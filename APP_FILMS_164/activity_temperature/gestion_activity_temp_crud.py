...
@app.route("/activity_temp_afficher/<int:id_film_sel>", methods=['GET', 'POST'])
def activity_temp_afficher(id_film_sel):
    print(" activity_season_afficher id_film_sel ", id_film_sel)
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT temp_name,name_activity,
            GROUP_CONCAT(DISTINCT activity_temperature_id ORDER BY activity_temperature_id) as ActivityTempIDs,
            GROUP_CONCAT(DISTINCT name_activity ORDER BY name_activity) as ActivityTemp
            FROM t_activity_temperature
            RIGHT JOIN t_temperature ON t_temperature.temp_id = t_activity_temperature.fk_temp
            LEFT JOIN t_activity ON t_activity.activity_id = t_activity_temperature.fk_activity
            GROUP BY 
            temp_name,name_activity;"""
                if id_film_sel == 0:
                    ...
                    strsql_genres_films_afficher_data += """ HAVING temp_id= %(value_id_film_selected)s"""
                ...
                if not data_genres_films_afficher and id_film_sel == 0:
                    flash("""La table "t_temperature" est vide. !""", "warning")
                elif not data_genres_films_afficher and id_film_sel > 0:
                    # Si l'utilisateur change l'temp_id dans l'URL et qu'il ne correspond à aucun film
            ...
            raise ExceptionFilmsGenresAfficher(f"fichier : {Path(__file__).name}  ;  {activity_temp_afficher.__name__} ;"
                                               f"{Exception_films_genres_afficher}")

    print("films_genres_afficher  ", data_genres_films_afficher)
    # Envoie la page "HTML" au serveur.
    return render_template("activity_temperature/activity_temp_afficher.html", data=data_genres_films_afficher)
...
"""
    ...
    1) Tous les genres contenus dans la "t_activity".
...
@app.route("/edit_activity_temp_selected", methods=['GET', 'POST'])
def edit_activity_temp_selected():
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
            ...
            # Récupère la valeur de "temp_id" du formulaire html "films_genres_afficher.html"
            # l'utilisateur clique sur le bouton "Modifier" et on récupère la valeur de "temp_id"
            # grâce à la variable "id_film_genres_edit_html" dans le fichier "films_genres_afficher.html"
            # href="{{ url_for('edit_genre_film_selected', id_film_genres_edit_html=row.temp_id) }}"
            ...
            data_genre_film_selected, data_genres_films_non_attribues, data_genres_films_attribues = \
                activity_temp_afficher_data(valeur_id_film_selected_dictionnaire)

            print(data_genre_film_selected)
            lst_data_film_selected = [item['temp_id'] for item in data_genre_film_selected]
            ...
            # Extrait les valeurs contenues dans la table "t_genres", colonne "name_activity"
            ...
            raise ExceptionEditGenreFilmSelected(f"fichier : {Path(__file__).name}  ;  "
                                                 f"{edit_activity_temp_selected.__name__} ; "
                                                 f"{Exception_edit_genre_film_selected}")

    return render_template("activity_temperature/activity_temp_modifier_tags_dropbox.html",
...
@app.route("/update_activity_temp_selected", methods=['GET', 'POST'])
def update_activity_temp_selected():
    if request.method == "POST":
        try:
            # Récupère l'id du film sélectionné
            id_film_selected = session['session_id_film_genres_edit']
            if not id_film_selected:
                flash("No film selected or session expired.", "error")
                return redirect(url_for('films_genres_afficher'))

            print("Film selected ID:", id_film_selected)
            ...
            # OM 2021.05.02 Une liste de "activity_id" qui doivent être effacés de la table intermédiaire "t_activity_temperature".
            ...
            # Une liste de "activity_id" qui doivent être ajoutés à la "t_activity_temperature"
            ...
            # "fk_temp"/"temp_id" et "fk_activity"/"activity_id" dans la "t_activity_temperature"
            strsql_insert_genre_film = """INSERT INTO t_activity_temperature (activity_temperature_id,fk_activity, fk_temp)
                                         VALUES (%(value_fk_genre)s, %(value_fk_film)s)"""

            # SQL pour effacer une (des) association(s) existantes entre "temp_id" et "activity_id" dans la "t_activity_temperature"
            strsql_delete_genre_film = """DELETE FROM t_activity_temperature WHERE fk_activity = %(value_fk_genre)s AND fk_temp = %(value_fk_film)s"""

            with DBconnection() as mconn_bd:
                # Pour le film sélectionné, parcourir la liste des genres à INSÉRER dans la "t_activity_temperature".
                ...
                # Pour le film sélectionné, parcourir la liste des genres à EFFACER dans la "t_activity_temperature".
                # Si la liste est vide, la boucle n'est pas parcourue.
            ...
            raise ExceptionUpdateGenreFilmSelected(f"fichier : {Path(__file__).name}  ;  "
                                                   f"{update_activity_temp_selected.__name__} ; "
                                                   f"{Exception_update_genre_film_selected}")

    # Après cette mise à jour de la table intermédiaire "t_activity_temperature",
    # on affiche les films et le(urs) genre(s) associé(s).
    return redirect(url_for('activity_temp_afficher', id_film_sel=id_film_selected))
...
def activity_temp_afficher_data(valeur_id_film_selected_dict):
    print("valeur_id_film_selected_dict...", valeur_id_film_selected_dict)
    try:

        strsql_film_selected = """SELECT temp_id, temp_name ,idActivityTemp, GROUP_CONCAT(activity_id) as ActivityTemp FROM t_activity_temperature
                                        INNER JOIN t_temperature ON t_temperature.temp_id = t_activity_temperature.fk_temp
                                        INNER JOIN t_activity ON t_activity.activity_id = t_activity_temperature.fk_activity
                                        WHERE temp_id = %(value_id_film_selected)s GROUP BY temp_id, temp_name, score
"""

        strsql_genres_films_non_attribues = """SELECT activity_id, name_activity FROM t_activity WHERE activity_id not in(SELECT activity_id as idActivityTemp FROM t_activity_temperature
                                                    INNER JOIN t_temperature ON t_temperature.temp_id = t_activity_temperature.fk_temp
                                                    INNER JOIN t_activity ON t_activity.activity_id = t_activity_temperature.fk_activity
                                                    WHERE temp_id = %(value_id_film_selected)s)"""

        strsql_genres_films_attribues = """SELECT temp_id, activity_id, name_activity ,idActivityTemp FROM t_activity_temperature
                                            INNER JOIN t_temperature ON t_temperature.temp_id = t_activity_temperature.fk_temp
                                            INNER JOIN t_activity ON t_activity.activity_id = t_activity_temperature.fk_activity
                                            WHERE temp_id = %(value_id_film_selected)s"""
        ...
        raise ExceptionGenresFilmsAfficherData(f"fichier : {Path(__file__).name}  ;  "
                                               f"{activity_temp_afficher_data.__name__} ; "
